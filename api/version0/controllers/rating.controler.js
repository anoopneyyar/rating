...
exports.getfeedback= async (req, res) => {
  try {
    let response = await ratingServices.findfeedbackById(req.params.entityid);
    res.status(200).send(responseHelper.createSuccessResponse(responseMessages.ratingfetch, response));
  } catch (error) {
    logger.error("Error occure while retrieving data" + error.message);
    res.status(error.httpcode || 500).send(error);
...
// Get average Rating of a post by entity ID
exports.getpostratingbyentity= async (req, res) => {
  try {
    let response = await ratingServices.findRatingByentity(req.params.entityid);
    res.status(200).send(responseHelper.createSuccessResponse(responseMessages.ratingfetch, response));
  } catch (error) {
    logger.error("Error occure while retrieving data" + error.message);
    res.status(error.httpcode || 500).send(error);
